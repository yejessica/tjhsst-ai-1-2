...
def max_value(state, turn, tc):
   ...
   v = -50000
   new_turn = turn if turn == 'X' else 'X'
   for a, s in successors(state, turn):
      # v = max(v, min_value(s, turn, tc)[0])
      nn, new_state = min_value(s, new_turn, tc)
      if v < nn:
         v, state = nn, s
...
def min_value(state, turn, tc):
   if terminal_test(state, tc): return (-1 * utility(turn, tc, state), state)

   v = 500000
   new_turn = turn if turn == 'O' else 'O'
   for a, s in successors(state, turn):
      # print(max_value(s, turn, tc)[0])
      nn, new_state = max_value(s, new_turn, tc)
      if v > nn:
         v, state = nn, s
   # return value and state: (val, state)4
