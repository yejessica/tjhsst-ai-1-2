...
def addWords(board, pos_words, height, width):
    print("hi")
    rowRef = []
    for i in range(height):
        rowRef.append(board[i*width:i*width+(width-1)])

    print(rowRef)

def main():
    inputTest = [r'^(\d+)x(\d+)$', r'^\d+$', r'^(H|V)(\d+)x(\d+)([a-zA-Z#]+)$']
