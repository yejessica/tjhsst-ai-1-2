...
def dist_heuristic(state, back):
   # Your code goes here
   # count = 0
   if back:
      return dist_heuristicBACK(state)
   else:
      return dist_heuristicFRONT(state)
...
def solve(start, goal="_123456789ABCDEF", heuristic=dist_heuristic, size = 4):
   ...
   heapq.heappush(f[0], (dist_heuristic(start, False), start, [start]))
   heapq.heappush(f[1], (dist_heuristic(goal, True), goal, [goal]))
      ...
      for i in range(2):
         ...
         for a in generate_children(t[1]):
            ...
            # newTuple = (sum(dist_heuristic(i) for i in t[2])+dist_heuristic(a), a, t[2]+newPath)
               ...
               newTuple = (len(t[2])+dist_heuristic(a, i), a, t[2]+newPath)
