...
def display_pathBFS(path_list, size):
...
def solve(start, goal="_123456789ABCDEF", heuristic=dist_heuristic, size = 4):
   ...
   heapq.heappush(frontier, (dist_heuristic(start), start, [start]))
   while len(frontier)>0 or len(Q)!=0:
      ...
      if t[1] == goal:
         return t[2] #the path
      if (s == start):
         #  return display_pathBFS(s, exploredBFS, start)
         return t[2][::-1]
      for ab in generate_children(s):
        if ab not in exploredBFS:
            Q.append(ab)
            exploredBFS[ab]=s
      for a in generate_children(t[1]):
         if a not in t[2]:
