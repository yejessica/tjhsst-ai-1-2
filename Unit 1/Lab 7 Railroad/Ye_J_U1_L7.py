# Name: Jessica Ye
# Data: 10/20/21
import random, pickle, math, time, heapq
...
def make_graph(nodes = "rrNodes.txt", node_city = "rrNodeCity.txt", edges = "rrEdges.txt"):
   ...
   with open(node_city,'r') as f:
      for line in f:
         s = line.split()
         city = line.replace(s[0], "").strip()
         nodeToCity[s[0]] = city
         cityToNode[city] = s[0]
   ...
   with open(edges, 'r') as f:
      ...
      for n in allEdges:
         s = n.split()
         edgeCost[(s[0], s[1])] = calc_edge_cost(*nodeLoc[s[0]], *nodeLoc[s[1]])
         edgeCost[(s[1], s[0])] = calc_edge_cost(*nodeLoc[s[0]], *nodeLoc[s[1]])
         if s[0] in neighbors:
            neighbors[s[0]] = neighbors.get(s[0]) + [s[1]]
         if s[1] in neighbors:
            neighbors[s[1]] = neighbors.get(s[1]) + [s[0]]
         if not s[0] in neighbors:
            neighbors[s[0]] = [s[1]]
         if not s[1] in neighbors:
            neighbors[s[1]] = [s[0]]
...
def dist_heuristic(n1, n2, graph):
   ...
   # Your code goes here
   # return calc_edge_cost(graph[4][(n1, n2)]) if (n1, n2) in graph[4] else calc_edge_cost(graph[4][(n2, n1)])
...
def generate_path(state, explored, graph):
   ...
   while explored[state] != "s":
      ...
      # print(prevPath)
      # print(state)
      if (prevPath, state) in graph[4]:
         cost += graph[4][(prevPath, state)]
      else:
         cost += graph[4][(state, prevPath)]
      prevPath = state
      state = explored[state]
...
def draw_final_path(ROOT, canvas, path, graph, col='red'):
   prevNode = graph[5][path[0]]
   for p in path[1::]:
      drawLine(canvas, *prevNode, *graph[5][p], col)
      prevNode = graph[5][p]
   # Your code goes here
...
def bi_bfs(start, goal, graph, col):
   ...
   while place[0] < len(queue[0]) or place[1] < len(queue[1]):
      for i in range(2):
         if place[i] >= len(queue[i]): continue
         s = queue[i][place[i]]
         place[i] += 1
         # if (s == goal and i == 0) or (s == start and i == 1): 
         #    path, cost = generate_path(s, explored[i], graph)
         #    draw_final_path(ROOT, canvas, path, graph)
         #    return path, cost
         for a in graph[3][s]:
            # explored[i][a] = s
            # queue[i].append(a)
            # drawLine(canvas, *graph[5][s], *graph[5][a], col)
            if a in explored[1-i].keys():
               path, cost = generate_path(s, explored[i], graph)
               path2, cost2 = generate_path(a, explored[1-i], graph)
               path = path+path2[::-1]
               cost = cost+cost2
               draw_final_path(ROOT, canvas, path, graph)
               return path[::-1], cost, counter
            if a not in explored[i].keys():
               queue[i].append(a)
               explored[i][a] = s
               drawLine(canvas, *graph[5][s], *graph[5][a], col)
         counter += 1
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
   ...
   # add = []
   frontier = []
   startTuple = (heuristic(start, goal, graph), start, [start])
   best = {start:0}
   explored = {start:"s"}
   ...
   while len(frontier) > 0:
      ...
      if t[1] == goal:
         ...
         cost = sum(calc_edge_cost(*graph[0][t[2][j-1]],*graph[0][t[2][j]]) for j in range(1, len(t[2])))
         draw_final_path(ROOT, canvas, t[2], graph)
         return t[2], cost, counter
      # org = calc_edge_cost
      # print(best)
      # print(best[t[1]])
      

      for a in graph[3][t[1]]:
         newHeur = best[t[1]]- dist_heuristic(t[1], goal, graph) + graph[4][(t[1], a)] + heuristic(a, goal, graph)
         # if a not in t[2]:
         if a not in explored.keys() or newHeur < best[a]:
            # newDist = heuristic(a, goal, graph) + calc_edge_cost(*graph[0][t[2][0]], *graph[0][a])
            
            newPath = [a]
            newTuple = (newHeur, a, t[2]+newPath)
         ...
         drawLine(canvas, *graph[5][t[1]], *graph[5][a], col)
         counter+=1
...
def bi_a_star(start, goal, graph, col, heuristic=dist_heuristic):
   ...
   startTuple = (heuristic(start, goal, graph), start)
   endTuple = (heuristic(goal, start, graph), goal)
   heapq.heappush(frontier[0], startTuple)
   heapq.heappush(frontier[1], endTuple)
   
   while 0 < len(frontier[0]) or 0 < len(frontier[1]):
      for i in range(2):
         # if place[i] >= len(frontier[i]): continue
         # s = frontier[i][place[i]]
         t = heapq.heappop(frontier[i])
         # place[i] += 1

         for a in graph[3][t[1]]:
            if a in explored[1-i].keys():
               path, cost = generate_path(t[1], explored[i], graph)
               path2, cost2 = generate_path(a, explored[1-i], graph)
               # path = t[2] + frontier
               path = path+path2[::-1]
               cost = cost+cost2
               # cost = sum(calc_edge_cost(*graph[0][path[j-1]],*graph[0][path[j]]) for j in range(1, len(path)))
               draw_final_path(ROOT, canvas, path, graph)
               return path[::-1], cost, counter

            if i == 0:
               newHeur = best[i][t[1]] - heuristic(t[1], goal, graph) + graph[4][(t[1], a)] + heuristic(a, goal, graph)
            else:
               newHeur = best[i][t[1]] - heuristic(t[1], start, graph) + graph[4][(t[1], a)] + heuristic(a, start, graph) 
            
            if a not in explored[i].keys() or newHeur < best[i][a]:

               newTuple = (newHeur, a)
               heapq.heappush(frontier[i], newTuple)
               best[i][a] = newHeur
               explored[i][a] = t[1]
               drawLine(canvas, *graph[5][t[1]], *graph[5][a], col)
         counter += 1
...
def tri_directional(city1, city2, city3, graph, col, heuristic=dist_heuristic):
   ...
   # Your code goes here
   ROOT = Tk()
   ROOT.title("Tri-Directional A-STAR")
   canvas = Canvas(ROOT, background='black') #sets background
   draw_all_edges(ROOT, canvas, graph)
   ...
   for j in range(3):
      ...
      path, cost, counter = tri_directionalHelper(ROOT, canvas, start, goal, graph, col, counter)
      pathCosts[cost] = path
      
   costs = [k for k in pathCosts.keys()]
   minCost1 = min(pathCosts.keys())
   # costs.remove(minCost1)
   costs.remove(minCost1)

   minCost2 = min(costs)
   finalPath = pathCosts[minCost1]+pathCosts[minCost2]

   finalCost = minCost1+minCost2
   draw_final_path(ROOT, canvas, finalPath[1:], graph)
   return finalPath[1:], finalCost, counter

   return None

def tri_directionalHelper(ROOT, canvas, start, goal, graph, col, counter, heuristic = dist_heuristic):
   best = [{start:0}, {goal:0}]
   # place = [0, 0]
   explored = [{start: "s"}, {goal: "s"}]
   frontier = [[], []]
...
def main():
   ...
   cur_time = time.time()
   path, cost, counter = bfs(graph[2][start], graph[2][goal], graph, 'yellow') #graph[2] is city to node
   if path != None: 
   ...
   cur_time = time.time()
   path, cost, counter = bi_bfs(graph[2][start], graph[2][goal], graph, 'green')
   if path != None: 
   ...
   cur_time = time.time()
   path, cost, counter = a_star(graph[2][start], graph[2][goal], graph, 'blue')
   if path != None:
   ...
   cur_time = time.time()
   path, cost, counter = bi_a_star(graph[2][start], graph[2][goal], graph, 'orange')
   if path != None:
   ...
   cur_time = time.time()
   path, cost, counter = tri_directional(graph[2][start], graph[2][goal], graph[2][third], graph, 'pink')
   if path != None: 

      print('The number of explored nodes of Tri-directional search:', counter)
      print('The whole path:', path)
      print('The length of the whole path:', len(path))
      print(display_path(path, graph))
      print ('Tri-directonal Bi-astar Path Cost:', cost)
      print ('Tri-direction Bi-A star duration:', (time.time() - cur_time))
      print ()
      # display_path(path, graph)
   else: print ("No Path Found.")
