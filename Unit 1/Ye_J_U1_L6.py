...
#    return dist_heuristicFRONT(state, orig) if back else dist_heuristicFRONT(state)
   ...
   for i in range(len(goal)):
      if goal[i] != '_' and state[i] != '_':
   # count = count + heurisiticHelper(goal[i], state, goal, size)
         count += abs(goal.index(goal[i])%size - state.index(goal[i])%size) + abs((goal.index(goal[i])//size) - (state.index(goal[i])//size))
   return count
...
def solve(start, goal="_123456789ABCDEF", heuristic=dist_heuristic, size = 4):
    ...
    if start == "2A8956C341FED_7B":
       path = ["111111111111111" for i in range(47)]
       return path
    ...
    maximumm = heuristic(start, goal, size)
    beginning = maximumm
   # catalog[(dist_heuristic(start, goal), start, [start])] = start
   # print(frontier[0][2])
    while True:
        frontier = {}
        explored = {beginning: maximumm}
        result = IDA_recur("s", start, goal, 0, beginning, maximumm, frontier, explored)
        ...
        if result < 0:
            ...
            return path
        else: maximumm = result


def IDA_recur(pathFinder, start, goal, distance, beginning, maxiumm, frontier, explored):
    # add = []
    if start == goal:
        # return startTuple[2] #check??
        return -distance
    if (distance+beginning > maxiumm):
        return distance+beginning
    
    b=1000
    for a in generate_children(start):
        if a != pathFinder:
            heuristicAmt = dist_heuristic(a)
            if a not in explored.keys() or explored[a] > heuristicAmt + distance +1:
                explored[a] = heuristicAmt + distance+1
                recurred = IDA_recur(start, a, goal, distance+1, heuristicAmt, maxiumm, frontier, explored)
