...
def a_star(start, goal, words_set):
   ...
   for i in range(1, 1000):
      result = IDA_recur(frontier, start, goal, i, words_set, tuplePushed)
...
def IDA_recur(frontier, start, goal, limit, words_set, startTuple):
   ...
   if startTuple[0] > limit:
      return None
   for a in generate_adjacents(start, words_set):
      if ((dist_heuristic(a, goal)+len(startTuple[2])) < limit) and a not in startTuple[2]:
         ...
         heapq.heappush(frontier, newTuple)
         result = IDA_recur(frontier, a, goal, limit, words_set, newTuple)
         # print(result)
